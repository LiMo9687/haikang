...
flags.DEFINE_string("video", r"D:\Code\PyCharm\Yolov5-Deepsort-main\test2.mp4", "Path to input video or webcam index (0)")
...
def initialize_model():
    model_path = r"D:\Code\PyCharm\runs\detect\train20\weights\best.pt"
...
def load_class_names():
    classes_path = "./configs/data.names"
    ...
    for track in tracks:
        ...
        if FLAGS.blur_id is not None and class_id == FLAGS.blur_id:
            if 0 <= x1 < x2 <= frame.shape[1] and 0 <= y1 < y2 <= frame.shape[0]:
                frame[y1:y2, x1:x2] = cv2.GaussianBlur(frame[y1:y2, x1:x2], (7, 7), 3)
